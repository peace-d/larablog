...
{{--                            <a href="{{ route('app_get_all_my_posts') }}" class="btn btn-success">Create New Post</a>--}}
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form class="row g-3" method="POST" action="{{ route('app_create_new_post') }}" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">Title</label>
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" placeholder="" value="{{ @old('title') }}" name="title">
                                    @error('title')
                                ...
                                <div class="mb-3">
                                    <textarea class="ckeditor form-control @error('content') is-invalid @enderror" name="content">{{ @old('content') }}</textarea>
                                    @error('content')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="col-12 text-end">
                                    <button type="submit" class="btn btn-sm btn-primary mb-0">Save Post</button>
                                </div>
                            </div>
                        </form>
                    </div>
